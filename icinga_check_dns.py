...
import nagiosplugin
from dnsviz.analysis import DNS_RAW_VERSION, TTLAgnosticOfflineDomainNameAnalysis
        ...
        if delegation["status"] > status.DELEGATION_STATUS_SECURE:
            if delegation["status"] == status.DELEGATION_STATUS_BOGUS:
                return nagiosplugin.Result(
                    nagiosplugin.Critical, "Delegation is BOGUS", metric
                )
            if delegation["status"] == status.DELEGATION_STATUS_INSECURE:
                if resource.insecure_ok:
                    return nagiosplugin.Result(
                        nagiosplugin.Ok, "Delegation is INSECURE", metric
                    )
            return nagiosplugin.Result(
                nagiosplugin.Warn,
                "Delegation is {}".format(
                    status.delegation_status_mapping[delegation["status"]]
                ),
                metric,
            )
...
class RRSIGExpitationContext(nagiosplugin.Context):
    ...
    def evaluate(self, metric, resource):
        ...
        return nagiosplugin.Result(
            r_state,
            f"RRSIGs expiring in {metric.value} seconds ({metric.value / 3600} hours)",
            metric,
        )
...
class DNS(nagiosplugin.Resource):
    def __init__(
        ...
        domain,
        insecure_ok=True,
        rrsig_expiration_warn=72,
        rrsig_expiration_crit=24,
    ):
        self.domain = domain
    ...
    def probe(self):
        _log.debug(f'Starting probing for "{self.domain}"')
        a = dnsviz.commands.probe.BulkAnalyst(
            self.rdclass,
            self.try_ipv4,
            self.try_ipv6,
            self.client_ipv4,
            self.client_ipv6,
            self.query_class_mixin,
            self.ceiling,
        ...
        analysis_obj = TTLAgnosticOfflineDomainNameAnalysis.deserialize(
            names[0], serialized_obj, cache
        )
        ...
        delegation_warnings = [
            w.description for w in analysis_obj.delegation_warnings[43]
        ]
            ...
            for rrsig, rrsets in rrsigs.items():
                for keymeta, single_rrsig_status in rrsets.items():
                    ...
                    for w in single_rrsig_status.warnings:
                        rrsig_warnings.add(f"{keymeta}: {w.description}")
                    for e in single_rrsig_status.errors:
                        rrsig_errors.add(f"{keymeta}: {e.description}")
        ...
        yield nagiosplugin.Metric(
            "rrsig_status",
            {"errors": rrsig_errors, "warnings": rrsig_warnings},
            context="rrsig",
        )
        yield nagiosplugin.Metric(
            "rrsig_expiration", rrsig_expiration, context="rrsig_expiration"
        )
...
def main():
    argp = argparse.ArgumentParser(
        prog="check_dns", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    argp.add_argument(
        "--domain",
        "-d",
    ...
    if args.expire_crit > args.expire_warn:
        raise nagiosplugin.CheckError(
            f"RRSIG Critical value ({args.expire_crit}) higher than Warning value ({args.expire_warn})"
        )
